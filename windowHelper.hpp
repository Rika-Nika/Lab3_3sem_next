...
void testDeikstra(HWND hWndOutput0, HWND hWndOutput1, HWND hWndOutput2,
	Alg alg, int startVert, int endVert, Matrix matrixEnum, int** matrix, int numberOfVert, std::wstring fileName)
{
	...
	deikstra<int>(matrix, numberOfVert, startVert, endVert, goodPath, countVer, minLength);
	auto endTime = std::chrono::system_clock::now();
	auto deltaDeikstra = std::chrono::duration_cast<std::chrono::microseconds>(endTime - startTime);

	oss2 << L"Время выполнения (микро сек): " << deltaDeikstra.count() << "\r\n";
	...
	str.append("Дейкстра;").append(std::to_string(numberOfVert)).append(";").append(std::to_string(deltaDeikstra.count())).append("\n");
	
	SetWindowText(hWndOutput1, oss2.str().c_str());
	writeToCSV(fileName.append(L"_").append(std::to_wstring(numberOfVert)), str);
	...
	HWND hFileName, Alg alg, HWND hStartVert, HWND hEndVert, Matrix matrixEnum)
{
	...
	int  numberOfVert;
	//матрица смежности
	int** matrix = loadMatrix<int>(fileName, numberOfVert);
	if (nullptr != matrix) {
	...
	SetWindowText(hWndOutput0, oss1.str().c_str());
	testDeikstra(hWndOutput0, hWndOutput1, hWndOutput2, alg, startVert, endVert, matrixEnum, matrix,numberOfVert, fileName);
}
